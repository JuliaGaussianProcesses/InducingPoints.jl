...
function init(alg::OIPS, X::AbstractVector; kernel::Kernel, kwargs...)
    N = length(X) # Number of samples
    N >= Z.kmin || throw(ArgumentError("First batch should have at least $(Z.kmin) samples"))
    samples = sample(1:N, floor(Int, Z.kmin); replace = false)
    Z = update!(X[samples], alg, X; kernel=kernel)
    return Z
end

function update!(Z::AbstractVector, alg::OIPS, X::AbstractVector; kernel::Kernel)
    add_point!(Z, alg, X, kernel)
end

function add_point!(Z::AbstractVector, alg::OIPS, X::AbstractVector, kernel::Kernel)
    b = length(X)
...
function remove_point!(Z::AbstractVector, alg::OIPS, K::AbstractMatrix)
